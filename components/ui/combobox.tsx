...
function ComboboxInput({
  ...
  return (
    <InputGroup className={cn("w-auto h-10 border border-gray-400", className)}>
      ...
      <InputGroupAddon align="inline-end">
        {showTrigger && (
          <InputGroupButton
            ...
            className="group-has-data-[slot=combobox-clear]/input-group:hidden text-xl data-pressed:bg-transparent"
            disabled={disabled}
          >
            <ComboboxTrigger />
          </InputGroupButton>
...
function ComboboxItem({
  ...
  return (
    <ComboboxPrimitive.Item
      ...
      className={cn(
        "relative flex min-h-10 w-full cursor-default items-center gap-5 px-2 py-2 text-sm outline-hidden select-none data-highlighted:bg-turquoise data-highlighted:font-semibold data-highlighted:cursor-pointer data-highlighted:text-white not-data-[variant=destructive]:data-highlighted:**:text-accent-foreground data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-3.5",
        className
      )}
      {...props}
    >
      ...
      >
        <HugeiconsIcon icon={Tick02Icon} strokeWidth={4} className="pointer-events-none" />
      </ComboboxPrimitive.ItemIndicator>
    </ComboboxPrimitive.Item>
  )
}
...
function ComboboxChip({
  ...
  return (
    <ComboboxPrimitive.Chip
      ...
      className={cn(
        "flex h-[calc(--spacing(4.75))] w-fit items-center justify-center gap-1 bg-muted-foreground/10 px-1.5 text-xs/relaxed font-medium whitespace-nowrap text-foreground has-disabled:pointer-events-none has-disabled:cursor-not-allowed has-disabled:opacity-50 has-data-[slot=combobox-chip-remove]:pr-0",
        className
      )}
      {...props}
    >
