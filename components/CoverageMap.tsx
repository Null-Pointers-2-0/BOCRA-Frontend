...
export default function CoverageMap({
  ...
  const districtKey = useMemo(
    () =>
      `districts-${showDistricts}-${showHeatMap}-${coverageGeoJSON?.features?.length}`,
  ...
  const districtStyle = useMemo(
    () =>
      (feature: GeoJSON.Feature | undefined): PathOptions => {
        if (!feature) return {};
        const code =
          (feature.properties as { code?: string })?.code || "";
  ...
  const coverageStyle = useMemo(
    ...
    [showHeatMap, selectedOperator, operatorColors]
  );
  ...
  const onEachDistrict = useMemo(
    () => (feature: GeoJSON.Feature, layer: Layer) => {
      ...
      layer.on("click", () => {
        if (props?.id && onDistrictClick) onDistrictClick(props.id);
      });
    },
