import { apiClient, setTokens, clearTokens } from "../client";
import type { ApiResponse } from "../types/common";
import type {
  User,
  LoginRequest,
  LoginResponse,
  RegisterRequest,
  PasswordResetRequest,
  PasswordResetConfirmRequest,
  PasswordChangeRequest,
  ProfileUpdateRequest,
} from "../types/auth";

export async function register(
  data: RegisterRequest
): Promise<ApiResponse<User>> {
  return apiClient<User>("/accounts/register/", {
    method: "POST",
    body: data,
  });
}

export async function login(
  data: LoginRequest
): Promise<ApiResponse<LoginResponse>> {
  const res = await apiClient<LoginResponse>("/accounts/login/", {
    method: "POST",
    body: data,
  });
  if (res.success && res.data?.tokens) {
    setTokens(res.data.tokens.access, res.data.tokens.refresh);
  }
  return res;
}

export async function logout(): Promise<ApiResponse<null>> {
  const res = await apiClient<null>("/accounts/logout/", { method: "POST" });
  clearTokens();
  return res;
}

export async function verifyEmail(
  token: string
): Promise<ApiResponse<null>> {
  return apiClient<null>("/accounts/verify-email/", { params: { token } });
}

export async function resendVerification(
  email: string
): Promise<ApiResponse<null>> {
  return apiClient<null>("/accounts/resend-verification/", {
    method: "POST",
    body: { email },
  });
}

export async function requestPasswordReset(
  data: PasswordResetRequest
): Promise<ApiResponse<null>> {
  return apiClient<null>("/accounts/password-reset/", {
    method: "POST",
    body: data,
  });
}

export async function confirmPasswordReset(
  data: PasswordResetConfirmRequest
): Promise<ApiResponse<null>> {
  return apiClient<null>("/accounts/password-reset/confirm/", {
    method: "POST",
    body: data,
  });
}

export async function changePassword(
  data: PasswordChangeRequest
): Promise<ApiResponse<null>> {
  return apiClient<null>("/accounts/change-password/", {
    method: "POST",
    body: data,
  });
}

export async function getProfile(): Promise<ApiResponse<User>> {
  return apiClient<User>("/accounts/profile/");
}

export async function updateProfile(
  data: ProfileUpdateRequest
): Promise<ApiResponse<User>> {
  return apiClient<User>("/accounts/profile/", {
    method: "PATCH",
    body: data,
  });
}
